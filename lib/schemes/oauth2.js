...
export default class Oauth2Scheme {
  ...
  get _localeRedirectURI () {
    const { i18n } = this.$auth.ctx.app
    let localePath = ''
  ...
  login ({ params, state, nonce } = {}) {
    const opts = {
      protocol: 'oauth2',
      // https://developers.facebook.com/docs/facebook-login/manually-build-a-login-flow/v2.2?locale=zh_TW#reaskperms
  ...
  async _handleCallback (uri) {
    ...
    if (this.$auth.options.redirect && this.$auth.ctx.route.path.indexOf(this.$auth.options.redirect.callback) < 0) {
      return
    }
    ...
    if (this.options.response_type === 'code' && parsedQuery.code) {
      const data = await this.$auth.request({
        ...
        data: encodeQuery({
          ...
          client_id: this.options.client_id,
          redirect_uri: this._localeRedirectURI,
          response_type: this.options.response_type,
